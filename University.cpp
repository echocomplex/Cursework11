...
List <std::pair<Student, float>> University::sort (const std::string group, const unsigned short session) {
    List <std::pair<Student, float>> result;
    for (int i = 0; i < this->database.length(); ++i) {
        if (this->database[i].getGroup() == group) {
            Student unit = this->database[i];
            float avg = 0;
            if ((session <= unit.getNumberOfSessions()) && (session > 0)) {
                for (int j = 0; j < unit.getSessions()[session - 1].length(); ++j) {
                    avg += unit.getSessions()[session - 1][j].mark;
                }
                avg /= unit.getSessions()[session - 1].length();
                result.append(std::make_pair(unit, avg));
            }
        }
    }
    if (result.length() > 0) {
        for (int i = 0; i < result.length() - 1; ++i) {
            for (int j = 0; j < result.length() - i - 1; ++j) {
                if (result[j].second < result[j + 1].second) {
                    std::pair<Student, float> temp = result[j];
                    result[j] = result[j + 1];
                    result[j + 1] = temp;
                }
            }
